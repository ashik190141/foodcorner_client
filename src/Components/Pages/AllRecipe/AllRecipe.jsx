...
const AllRecipe = () => {
  ...
  useEffect(() => {
    fetch("https://foodcorner-omega.vercel.app/all-recipe")
      .then((res) => res.json())
      .then((data) => {
        setShow(false);
        console.log(data);
  ...
  const handleCategory = (value) => {
    ...
    if (searchText == "all") {
      ...
      setRecords(
        recipes.filter((recipe) => recipe.category.toLowerCase() == searchText)
      );
    }
  };
  ...
  const handleCountry = (value) => {
    ...
    if (searchText == "all") {
      ...
      setRecords(
        recipes.filter((recipe) => recipe.country.toLowerCase() == searchText)
      );
    }
  };
            ...
            >
              ...
              {recipesCategory?.map((category) => (
                ...
                >
                  <p
                    className={`${
                      text == category.category ? "text-blue-600 font-bold" : ""
                    }`}
                  >
                    {category.category}
                  </p>
